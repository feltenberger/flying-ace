  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative z-10 rounded-xl bg-slate-800 border border-slate-600 shadow-2xl shadow-black/50 w-full max-w-lg my-8">
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-700">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-100 text-sm px-3 py-1 rounded border border-slate-600 hover:border-slate-500 transition-colors"
          >
            Close
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-6 text-sm text-slate-300 leading-relaxed">
          ...
          <Section title="Turn Structure">
            <ol className="list-decimal list-inside space-y-1 text-slate-400">
              <li><span className="text-slate-300">Roll the die</span></li>
              <li><span className="text-slate-300">Shop (buy one item or skip)</span></li>
              <li><span className="text-slate-300">Pay fuel tax</span></li>
              <li><span className="text-slate-300">End turn</span></li>
            </ol>
          </Section>
          ...
          <Section title="Shop Items">
            <div className="space-y-2">
              <Item name="1 Plane" cost="10f">Gain 1 plane.</Item>
              <Item name="2-Plane Pack" cost="15f">Gain 2 planes.</Item>
              <Item name="Dig for Fuel" cost="1f">Roll a die and gain that much fuel (6 = 10).</Item>
              <Item name="Insurance" cost="10f">Protects against plane loss for 3 turns (die rolls, dog fights, tax). Does NOT block Pricey Bombs. One purchase per game.</Item>
          ...
          <Section title="Protection Priority">
            ...
            <ol className="list-decimal list-inside space-y-1 text-slate-400 mt-1">
              <li><span className="text-slate-300">Anti-Aircraft &mdash; blocks all hit types (consumed on use)</span></li>
              <li><span className="text-slate-300">Insurance &mdash; blocks all except Pricey Bombs</span></li>
              <li><span className="text-slate-300">Otherwise &mdash; plane is lost</span></li>
            </ol>
          </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="text-sm font-semibold text-amber-400 uppercase tracking-wider mb-2">{title}</h3>
      {children}
    </div>
  )
}
...
function Roll({ n }: { n: string }) {
  return <span className="inline-block w-6 text-center font-mono font-bold text-slate-100">{n}</span>
}

function Item({ name, cost, children }: { name: string; cost: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="flex justify-between items-baseline">
        <span className="font-semibold text-slate-200">{name}</span>
        <span className="text-xs font-mono text-amber-400 ml-2">{cost}</span>
      </div>
